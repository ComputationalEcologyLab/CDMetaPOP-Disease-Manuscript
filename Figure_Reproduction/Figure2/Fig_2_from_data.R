...
input_dir <- "Fig2_summary_data"

# Define where figure outputs should go
output_dir <- "figure_outputs"


# SIRD Pop Size (Fig 2a) ----------------------------------------------------

# Read in data
Fig_2a_data<- readRDS(paste0(input_dir, "/Fig_2a_data.Rds"))

# .....Plot SIRD
SIRD_fig <- ggplot(Fig_2a_data, aes(x = Year, y = mean, 
                                    color = Scenario, shape = Scenario, fill = Scenario)) +
  geom_line(linewidth = 0.5) +
  geom_point(data = Fig_2a_data %>% filter(Year %% 10 == 0), 
             size = 2, color = "black") +
...
# Allele frequencies (Fig 2b) ------------------------------------------------------
# Read in data
Fig_2b_data<- readRDS(paste0(input_dir, "/Fig_2b_data.Rds"))

# Plot

Allele_Freq <- ggplot(Fig_2b_data, aes(x = Year, y = mean, color = Allele, fill = Allele)) +
...
# Combine Fig 2a and 2b  -----------------------------------------------------------

library(patchwork)

png(filename = paste0(output_dir, "/fig2_combined.png"),
